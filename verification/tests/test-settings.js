...
const src = fs.readFileSync(require('./paths').CLIENT_SRC, 'utf8')
{
  ...
  if (m) {
    ...
    ok(catCt === 43, '控件 43 项（字体5+布局3+颜色·编辑器12+颜色·语法10+颜色·界面8+颜色·交互5）', catCt)
    ...
    ok(ids.filter((x) => x.group === "颜色·语法").length === 10, '颜色·语法 10 项', ids.filter((x) => x.group === "颜色·语法").length)
    ...
    ok(ids.filter((x) => x.type === "color").length === 35, 'color 类型 35 项', ids.filter((x) => x.type === "color").length)
