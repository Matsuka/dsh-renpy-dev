# DSH Ren'Py 开发工作台（dsh-renpy-dev）— 贡献指南

感谢你考虑为 DSH Ren'Py 开发工作台做贡献。这个项目以**知识质量**为立身之本，
...
- **不复制/移植任何现成代码**：包括 renpy-mcp、VSCode 扩展、GitHub Actions、或其他任何开源项目的源码与实现。借鉴**思想/设计模式**（如"写守卫管线""诊断工具化"）可以，但所有代码必须是我们自己的原创实现。
- **引用来源**：若某设计理念明确来自外部项目，在注释或文档中注明出处与借鉴点，但**不得逐行/结构性照搬**。
- **许可证合规**：本项目为 MIT；任何可能涉及外部代码的场景（复用、移植、参考实现）先确认许可证兼容性，AGPL/GPL 等传染性许可的代码一律不采用。
- **目的**：保证代码可独立发布（MIT）、无法律风险、体现本项目自身工程实践。
